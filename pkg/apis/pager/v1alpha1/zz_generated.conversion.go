...
func autoConvert_v1alpha1_TestRunList_To_pager_TestRunList(in *TestRunList, out *pager.TestRunList, s conversion.Scope) error {
	...
	if in.Items != nil {
		...
		*out = make([]pager.TestRun, len(*in))
		for i := range *in {
			if err := Convert_v1alpha1_TestRun_To_pager_TestRun(&(*in)[i], &(*out)[i], s); err != nil {
				return err
			}
		}
	} else {
...
func autoConvert_pager_TestRunList_To_v1alpha1_TestRunList(in *pager.TestRunList, out *TestRunList, s conversion.Scope) error {
	...
	if in.Items != nil {
		...
		*out = make([]TestRun, len(*in))
		for i := range *in {
			if err := Convert_pager_TestRun_To_v1alpha1_TestRun(&(*in)[i], &(*out)[i], s); err != nil {
				return err
			}
		}
	} else {
